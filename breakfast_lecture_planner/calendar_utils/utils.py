...
def get_weeks_in_year(year):
    """Определяет количество недель в году. Если 1 января — пятница, суббота или воскресенье,
    то оно относится к предыдущему году, и первая неделя начинается с ближайшего понедельника."""
    ...
    if last_weekday >= 3:  # Если 31 декабря — среда и далее, год заканчивается в этом году
        end_of_last_week = last_day_of_year + timedelta(days=6 - last_weekday)
    else:  # Если 31 декабря — понедельник-вторник, последняя неделя относится к следующему году
        end_of_last_week = last_day_of_year - timedelta(days=last_weekday + 1)

    # Разница в днях между последней и первой неделями, делённая на 7 даёт количество недель
    num_weeks = (end_of_last_week - start_of_first_week).days // 7 + 1

    return num_weeks

def get_start_and_end_dates(week_number, year):
    """Вычисляет даты начала (понедельник) и конца (воскресенье) недели по номеру недели и году"""
    first_day_of_year = date(year, 1, 1)
    first_weekday = first_day_of_year.weekday()
...
def get_next_week_number(year):
    ...
    if last_week_event:
        ...
        if next_week_number > get_weeks_in_year(year):  # Предполагается, что эта функция возвращает количество недель в году
            return 1
