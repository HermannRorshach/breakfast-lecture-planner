...
from planner.tasks import send_admin_notification
...
def send_participants_notification():
    ...
    if response.status_code == 200:
        ...
        if data:
            ...
            message += (
                f"Всего зарегистрированных участников обеда: {len(data)}\n\n"
    ...
    if response.status_code == 200:
        ...
        if data:
            ...
            data_sorted = sorted(
                data, key=lambda x: datetime.fromisoformat(x["date"]), reverse=True
            )[:3]
            feedbacks_list = "\n\n".join(
                [
                    f"{f['id']}: {f['name']}\nemail: {f['email']}\n"
